...
- Name: stages.spec.ts >> Elango Surfers - Stage Transitions >> version number is displayed
- Location: tests/stages.spec.ts:33:3
...
```ts
  ...
  2  | 
  3  | test.describe('Elango Surfers - Stage Transitions', () => {
  4  |   
  5  |   test('stage indicator shows current stage', async ({ page }) => {
  6  |     await page.goto('/');
  7  |     await page.click('button:has-text("TAP TO START")');
  8  |     await page.waitForTimeout(2000);
  9  |     
  10 |     const stageIndicator = page.locator('#stage-indicator');
  11 |     await expect(stageIndicator).toBeVisible();
  12 |     
  13 |     const stageText = await stageIndicator.textContent();
  14 |     console.log('Stage indicator:', stageText);
  15 |     
  16 |     expect(stageText).toContain('STAGE');
  17 |   });
  18 | 
  19 |   test('game runs for at least 30 seconds without crashing', async ({ page }) => {
  20 |     await page.goto('/');
  21 |     await page.click('button:has-text("TAP TO START")');
  22 |     
  23 |     // Wait 30 seconds
  24 |     await page.waitForTimeout(30000);
  25 |     
  26 |     // Game should still be running (no crash)
  27 |     const score = await page.locator('#score').textContent();
  28 |     console.log('Score after 30s:', score);
  29 |     
  30 |     expect(score).toBeDefined();
  31 |   });
  32 | 
  33 |   test('version number is displayed', async ({ page }) => {
  34 |     await page.goto('/');
  35 |     
> 36 |     const version = await page.locator('#version').textContent();
     |                                                    ^ Error: locator.textContent: Test timeout of 30000ms exceeded.
  37 |     console.log('Version:', version);
  38 |     
  39 |     expect(version).toContain('v5.');
  40 |   });
  41 | });
  42 | 
```