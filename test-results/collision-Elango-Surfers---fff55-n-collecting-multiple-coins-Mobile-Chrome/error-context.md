...
- Name: collision.spec.ts >> Elango Surfers - Collision & Game Over >> combo counter appears when collecting multiple coins
- Location: tests/collision.spec.ts:36:3
...
```ts
  ...
  2  | 
  3  | test.describe('Elango Surfers - Collision & Game Over', () => {
  4  |   
  5  |   test('game over screen appears after collision', async ({ page }) => {
  6  |     await page.goto('/');
  7  |     await page.click('button:has-text("TAP TO START")');
  8  |     await page.waitForTimeout(2000);
  9  |     
  10 |     // Wait for game over (could take a while, so we'll just check UI exists)
  11 |     const gameOverScreen = page.locator('#game-over');
  12 |     const gameOverVisible = await gameOverScreen.count() > 0;
  13 |     
  14 |     // Game over screen should exist in DOM (may not be visible yet)
  15 |     console.log('Game over screen exists:', gameOverVisible);
  16 |   });
  17 | 
  18 |   test('score increases during gameplay', async ({ page }) => {
  19 |     await page.goto('/');
  20 |     await page.click('button:has-text("TAP TO START")');
  21 |     
  22 |     // Wait for initial score
  23 |     await page.waitForTimeout(3000);
  24 |     const score1 = await page.locator('#score').textContent();
  25 |     console.log('Score at 3s:', score1);
  26 |     
  27 |     // Wait more
  28 |     await page.waitForTimeout(5000);
  29 |     const score2 = await page.locator('#score').textContent();
  30 |     console.log('Score at 8s:', score2);
  31 |     
  32 |     // Score should increase
  33 |     expect(score2).not.toBe(score1);
  34 |   });
  35 | 
  36 |   test('combo counter appears when collecting multiple coins', async ({ page }) => {
  37 |     await page.goto('/');
> 38 |     await page.click('button:has-text("TAP TO START")');
     |                ^ Error: page.click: Test timeout of 30000ms exceeded.
  39 |     
  40 |     // Wait for potential combo
  41 |     await page.waitForTimeout(10000);
  42 |     
  43 |     const combo = page.locator('#combo');
  44 |     const comboVisible = await combo.count() > 0;
  45 |     console.log('Combo visible:', comboVisible);
  46 |     
  47 |     if (comboVisible) {
  48 |       const comboText = await combo.textContent();
  49 |       console.log('Combo text:', comboText);
  50 |     }
  51 |   });
  52 | 
  53 |   test('high score persists', async ({ page }) => {
  54 |     await page.goto('/');
  55 |     
  56 |     // High score should be visible
  57 |     const highScore = await page.locator('#highscore').textContent();
  58 |     console.log('High score:', highScore);
  59 |     
  60 |     expect(highScore).toContain('High Score:');
  61 |   });
  62 | });
  63 | 
```