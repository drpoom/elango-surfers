import { onMounted, onUnmounted } from 'vue';
import * as THREE from 'three';
import { STAGES } from '../data/stages.js';
import {
  laneWidth,
  jumpStrength,
  slideDuration,
  FLY_LIFT,
  FLY_GRAVITY,
  FLY_MAX_HEIGHT,
  MIC_THRESHOLD,
  MIC_PEAK_THRESHOLD,
  minSwipeDistance,
  TILT_LR_THRESHOLD,
  TILT_LANE_COOLDOWN,
  CALIBRATION_MAX_SAMPLES
} from '../gameConstants.js';

export function useGameControls({
  getCtx,
  getSurfaceY,
  getSurfaceTilt,
  getCurveX,
  getCurveSlope,
  getMicVolume,
  playSound,
  triggerGameOver,
  startCountdown,
  createFloatingText,
  initAudio,
  tryStartBGMFromGesture,
  pauseGame,
  resumeGame,
  toggleSettings,
  gameSpawns
}) {
  const gravity = 0.015;

  const handleJump = () => {
    const ctx = getCtx();
    if (ctx.isJumping || ctx.isSliding || ctx.isFlying) return;
    ctx.isJumping = true;
    ctx.jumpVelocity = jumpStrength;
    playSound('jump');
  };

  const handleSlide = () => {
    const ctx = getCtx();
    if (ctx.isJumping || ctx.isSliding || ctx.isFlying) return;
    ctx.isSliding = true;
    ctx.slideTimer = slideDuration;
    playSound('jump');
  };

  const handleSwipe = (direction) => {
    const ctx = getCtx();
    if (ctx.gameOver || ctx.countdownLocked) return;
    if (direction === 'left') {
      if (ctx.currentLane > 0) ctx.currentLane--;
    } else if (direction === 'right') {
      if (ctx.currentLane < 2) ctx.currentLane++;
    } else if (direction === 'up') {
      handleJump();
    } else if (direction === 'down') {
      handleSlide();
    }
  };

  const handleTouchStart = (e) => {
    if (e.target.closest('#mute-btn, #tilt-btn, #mic-btn, #settings-btn, #settings-panel, #name-entry')) return;
    e.preventDefault();
    const ctx = getCtx();
    ctx.touchStartX = e.touches[0].clientX;
    ctx.touchStartY = e.touches[0].clientY;
    initAudio();
    tryStartBGMFromGesture();
  };

  const handleTouchEnd = (e) => {
    if (e.target.closest('#mute-btn, #tilt-btn, #mic-btn, #settings-btn, #settings-panel, #name-entry')) return;
    e.preventDefault();
    const ctx = getCtx();
    const touchEndXLocal = e.changedTouches[0].clientX;
    const touchEndYLocal = e.changedTouches[0].clientY;
    
    if (ctx.gameOver) {
      if (ctx.showNameEntry) return;
      if (Date.now() - ctx.gameOverTime < 1000) return;
      startCountdown();
      return;
    }
    
    const diffX = touchEndXLocal - ctx.touchStartX;
    const diffY = touchEndYLocal - ctx.touchStartY;
    
    if (Math.abs(diffX) > Math.abs(diffY)) {
      if (Math.abs(diffX) > minSwipeDistance) {
        if (diffX > 0) handleSwipe('right');
        else handleSwipe('left');
      }
    } else {
      if (Math.abs(diffY) > minSwipeDistance) {
        if (diffY < 0) handleSwipe('up');
        else handleSwipe('down');
      }
    }
  };

  const handleDeviceOrientation = (e) => {
    const ctx = getCtx();
    if (!ctx.tiltEnabled || ctx.gameOver) return;
    const beta = e.beta;
    const gamma = e.gamma;
    
    if (beta === null || gamma === null) return;
    
    ctx.lastBeta = beta;
    ctx.lastGamma = gamma;
    
    if (ctx.isCalibrating) {
      ctx.tiltCalibrationSamples.push({ beta, gamma });
      if (ctx.tiltCalibrationSamples.length > CALIBRATION_MAX_SAMPLES) {
        ctx.tiltCalibrationSamples.shift();
      }
      return;
    }
    
    if (ctx.tiltInitialBeta === null) {
      ctx.tiltInitialBeta = beta;
      ctx.tiltInitialGamma = gamma;
      return;
    }
    
    const tiltSideways = gamma - ctx.tiltInitialGamma;
    const now = Date.now();
    if (now - ctx.lastLaneChangeTime > TILT_LANE_COOLDOWN) {
      if (tiltSideways < -TILT_LR_THRESHOLD) {
        if (ctx.currentLane > 0) {
          ctx.currentLane--;
          ctx.lastLaneChangeTime = now;
        }
      } else if (tiltSideways > TILT_LR_THRESHOLD) {
        if (ctx.currentLane < 2) {
          ctx.currentLane++;
          ctx.lastLaneChangeTime = now;
        }
      }
    }
  };

  const handleKeyDown = (e) => {
    if (e.repeat) return;
    const ctx = getCtx();
    if (ctx.countdownLocked) return;

    // Shift + Shortcuts for Testing
    if (e.shiftKey) {
      const keyUpper = e.key.toUpperCase();
      if (keyUpper === 'G') {
        ctx.godMode = !ctx.godMode;
        console.log('GOD MODE:', ctx.godMode);
        createFloatingText(ctx.godMode ? 'GOD MODE ON' : 'GOD MODE OFF', new THREE.Vector3(0, 3, -10), '#00ffff');
        return;
      }
      if (keyUpper === 'S') {
        const stage = STAGES[ctx.currentStage];
        const currentStageDuration = stage.stageDuration || 30;
        ctx.stageTime = currentStageDuration;
        console.log('STAGE SKIPPED, stageTime set to:', ctx.stageTime);
        createFloatingText('STAGE SKIP!', new THREE.Vector3(0, 3, -10), '#ffff00');
        return;
      }
      if (keyUpper === 'B') {
        const stage = STAGES[ctx.currentStage];
        const currentStageDuration = stage.stageDuration || 30;
        ctx.stageTime = currentStageDuration;
        ctx.bossWarning = false;
        console.log('BOSS TRIGGERED IMMEDIATELY');
        createFloatingText('BOSS FIGHT!', new THREE.Vector3(0, 3, -10), '#ff0000');
        return;
      }
      if (keyUpper === 'O') {
        if (gameSpawns) {
          gameSpawns.spawnBonusPortal('showroom');
          console.log('BONUS PORTAL SPAWNED');
          createFloatingText('PORTAL!', new THREE.Vector3(0, 3, -10), '#ff00ff');
        }
        return;
      }
      if (keyUpper === 'P') {
        if (gameSpawns) {
          // Spawn shield powerup specifically
          const lane = Math.floor(Math.random() * 3);
          const laneX = (lane - 1) * laneWidth;
          const powerupGroup = new THREE.Group();
          const orbGeo = new THREE.SphereGeometry(0.5, 16, 16);
          const orbMat = new THREE.MeshToonMaterial({ 
            color: 0x00bfff, 
            emissive: 0x00bfff, 
            emissiveIntensity: 0.5,
            transparent: true,
            opacity: 0.8
          });
          const orb = new THREE.Mesh(orbGeo, orbMat);
          powerupGroup.add(orb);
          
          const ringGeo = new THREE.TorusGeometry(0.65, 0.06, 8, 24);
          const ringMat = new THREE.MeshBasicMaterial({ color: 0x00ffff });
          const ring = new THREE.Mesh(ringGeo, ringMat);
          ring.rotation.x = Math.PI / 2;
          powerupGroup.add(ring);
          
          const coneGeo = new THREE.ConeGeometry(0.25, 0.4, 4);
          const coneMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
          const cone = new THREE.Mesh(coneGeo, coneMat);
          cone.rotation.x = Math.PI;
          powerupGroup.add(cone);

          const safeZ = gameSpawns.findSafeZ ? gameSpawns.findSafeZ(lane, -50, 6) : -50;
          powerupGroup.position.set(laneX, 1 + getSurfaceY(safeZ), safeZ);
          powerupGroup.userData = { type: 'shield' };
          powerupGroup.baseY = 1;
          ctx.scene.add(powerupGroup);
          ctx.powerups.push({ mesh: powerupGroup, lane, type: 'shield', collected: false });
          console.log('SHIELD POWERUP SPAWNED');
          createFloatingText('SHIELD SPAWNED!', new THREE.Vector3(0, 3, -10), '#00bfff');
        }
        return;
      }
    }
    
    // Debug mode cheats
    if (ctx.debugMode) {
      if (e.key === 'g' || e.key === 'G') {
        ctx.godMode = !ctx.godMode;
        return;
      }
      if (e.key === 'o' || e.key === 'O') {
        ctx.score += 1000;
        return;
      }
      if (e.key === 'k' || e.key === 'K') {
        triggerGameOver();
        return;
      }
    }
    
    // Easter eggs/Debug cheats activation
    if (!ctx.debugMode) {
      ctx.debugKeyBuffer += e.key.toLowerCase();
      if (ctx.debugKeyTimer) clearTimeout(ctx.debugKeyTimer);
      ctx.debugKeyTimer = setTimeout(() => { ctx.debugKeyBuffer = ''; }, 2000);
      if (ctx.debugKeyBuffer.endsWith('debug')) {
        ctx.debugMode = true;
        console.log('🐛 DEBUG CHEATS ENABLED! Press G (God), O (Score+1000), K (Die)');
      }
    }

    if (ctx.gameOver) {
      if (e.key === ' ' || e.key === 'Spacebar') {
        if (ctx.showNameEntry) return;
        if (Date.now() - ctx.gameOverTime < 1000) return;
        startCountdown();
      }
      return;
    }
    
    if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A') {
      if (ctx.currentLane > 0) ctx.currentLane--;
    } else if (e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D') {
      if (ctx.currentLane < 2) ctx.currentLane++;
    } else if (e.key === 'ArrowUp' || e.key === 'w' || e.key === 'W' || e.key === ' ') {
      handleJump();
    } else if (e.key === 'ArrowDown' || e.key === 's' || e.key === 'S') {
      handleSlide();
    }
  };

  const updatePhysics = (delta, gameSpeed) => {
    const ctx = getCtx();
    const player = ctx.player;
    if (!player) return;

    // Voice fly controls (blow to fly)
    if (ctx.micEnabledRef && !ctx.gameOver) {
      const vol = getMicVolume();
      ctx.lastMicVolume = vol;
      if (vol > MIC_THRESHOLD) {
        if (!ctx.isFlying) {
          ctx.isFlying = true;
          ctx.isJumping = false;
          ctx.isSliding = false;
          ctx.flyVelocity = 0;
        }
        const liftFactor = Math.min((vol - MIC_THRESHOLD) / (MIC_PEAK_THRESHOLD - MIC_THRESHOLD), 1.5);
        ctx.flyVelocity += FLY_LIFT * liftFactor;
      }
    }

    const surfaceY = getSurfaceY(player.position.z);

    // Find animated limbs
    const leftArm = player.getObjectByName('left-arm');
    const rightArm = player.getObjectByName('right-arm');
    const leftLeg = player.getObjectByName('left-leg');
    const rightLeg = player.getObjectByName('right-leg');
    const headGroup = player.getObjectByName('head-group');

    if (ctx.isFlying) {
      ctx.flyVelocity += FLY_GRAVITY;
      player.position.y += ctx.flyVelocity;
      
      if (player.position.y <= 0.5 + surfaceY) {
        player.position.y = 0.5 + surfaceY;
        ctx.isFlying = false;
        ctx.flyVelocity = 0;
      } else if (player.position.y > FLY_MAX_HEIGHT + surfaceY) {
        player.position.y = FLY_MAX_HEIGHT + surfaceY;
        ctx.flyVelocity = 0;
      }

      // Fly pose - arms spread out like wings
      if (leftArm) { leftArm.rotation.z = -1.5; leftArm.rotation.x = 0; }
      if (rightArm) { rightArm.rotation.z = 1.5; rightArm.rotation.x = 0; }
      if (leftLeg) { leftLeg.rotation.x = 0.3; leftLeg.rotation.z = 0; }
      if (rightLeg) { rightLeg.rotation.x = 0.3; rightLeg.rotation.z = 0; }
      player.scale.y = 1.0;
    } else if (ctx.isJumping) {
      player.position.y += ctx.jumpVelocity;
      ctx.jumpVelocity -= gravity;
      if (player.position.y <= 0.5 + surfaceY) {
        player.position.y = 0.5 + surfaceY;
        ctx.isJumping = false;
        ctx.jumpVelocity = 0;
      }

      // Jump pose
      if (leftArm) { leftArm.rotation.x = -1.2; leftArm.rotation.z = 0; }
      if (rightArm) { rightArm.rotation.x = -1.2; rightArm.rotation.z = 0; }
      if (leftLeg) { leftLeg.rotation.x = 0.5; leftLeg.rotation.z = 0; }
      if (rightLeg) { rightLeg.rotation.x = 0.5; rightLeg.rotation.z = 0; }
      player.scale.y = 1.0;
    } else if (ctx.isSliding) {
      ctx.slideTimer -= delta;
      
      // Slide pose
      if (leftArm) { leftArm.rotation.x = 0.8; leftArm.rotation.z = 0; }
      if (rightArm) { rightArm.rotation.x = 0.8; rightArm.rotation.z = 0; }
      if (leftLeg) { leftLeg.rotation.x = -1.0; leftLeg.rotation.z = 0; }
      if (rightLeg) { rightLeg.rotation.x = -1.0; rightLeg.rotation.z = 0; }
      
      player.scale.y = 0.5;
      player.position.y = 0.3 + surfaceY;
      
      if (ctx.slideTimer <= 0) {
        ctx.isSliding = false;
        player.scale.y = 1.0;
        player.position.y = 0.5 + surfaceY;
      }
    } else {
      // Running swing animation
      const runSpeed = 8 + gameSpeed * 10;
      const time = ctx.clock ? ctx.clock.getElapsedTime() : Date.now() * 0.001;
      const swing = Math.sin(time * runSpeed) * 0.6;
      
      if (leftArm) { leftArm.rotation.x = swing; leftArm.rotation.z = 0; }
      if (rightArm) { rightArm.rotation.x = -swing; rightArm.rotation.z = 0; }
      if (leftLeg) { leftLeg.rotation.x = -swing * 0.8; leftLeg.rotation.z = 0; }
      if (rightLeg) { rightLeg.rotation.x = swing * 0.8; rightLeg.rotation.z = 0; }
      
      player.position.y = 0.5 + surfaceY + Math.abs(Math.sin(time * runSpeed)) * 0.05;
      player.scale.y = 1.0;
    }

    if (ctx.isSlippery) {
      ctx.slipperyTimer -= delta;
      if (ctx.slipperyTimer <= 0) {
        ctx.isSlippery = false;
      }
    }
    
    const playerLaneOffset = getCurveX(player.position.z);
    const targetLaneX = (ctx.currentLane - 1) * laneWidth + playerLaneOffset;
    const laneShiftSpeed = ctx.isSlippery ? 0.05 : 0.25;
    player.position.x += (targetLaneX - player.position.x) * laneShiftSpeed;
    
    const moveDir = targetLaneX - player.position.x;
    
    // Rotate player's Y-axis to face the road curve direction + body turn
    const bodyTurn = THREE.MathUtils.clamp(moveDir * 0.15, -0.3, 0.3);
    const curveSlope = getCurveSlope(player.position.z);
    player.rotation.y = THREE.MathUtils.lerp(player.rotation.y, Math.PI + bodyTurn + curveSlope, 0.08);
    player.rotation.z = THREE.MathUtils.lerp(player.rotation.z, moveDir * -0.08, 0.1);
    player.rotation.x = getSurfaceTilt(player.position.z);

    // Head faces movement direction slightly
    if (headGroup) {
      const targetHeadRotY = THREE.MathUtils.clamp(moveDir * -0.5, -0.6, 0.6);
      headGroup.rotation.y = THREE.MathUtils.lerp(headGroup.rotation.y, targetHeadRotY, 0.1);
    }
  };

  let iosTiltPermissionRequested = false;
  const requestTiltPermission = async () => {
    const ctx = getCtx();
    if (iosTiltPermissionRequested) return;
    iosTiltPermissionRequested = true;
    try {
      const state = await DeviceOrientationEvent.requestPermission();
      if (state === 'granted') {
        window.addEventListener('deviceorientation', handleDeviceOrientation);
        ctx.tiltEnabled = true;
        ctx.tiltEnabledRef = true;
      } else {
        ctx.tiltEnabled = false;
        ctx.tiltEnabledRef = false;
      }
    } catch (err) {
      console.log('iOS tilt permission error:', err);
      ctx.tiltEnabled = false;
      ctx.tiltEnabledRef = false;
    }
  };

  const toggleTilt = async () => {
    const ctx = getCtx();
    if (window.DeviceOrientationEvent && typeof DeviceOrientationEvent.requestPermission === 'function') {
      if (!ctx.tiltEnabled) {
        iosTiltPermissionRequested = false;
        await requestTiltPermission();
      } else {
        ctx.tiltEnabled = false;
        ctx.tiltEnabledRef = false;
        ctx.tiltInitialBeta = null;
      }
    } else {
      ctx.tiltEnabled = !ctx.tiltEnabled;
      ctx.tiltEnabledRef = ctx.tiltEnabled;
      ctx.tiltInitialBeta = null;
    }
  };

  onMounted(() => {
    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('touchstart', handleTouchStart, { passive: false });
    window.addEventListener('touchend', handleTouchEnd, { passive: false });
    
    const preventScroll = (e) => {
      const ctx = getCtx();
      if (e.target.closest('#mute-btn, #tilt-btn, #mic-btn, #settings-btn, #settings-panel')) return;
      if (ctx.touchStartX !== null && Math.abs(e.touches[0].clientX - ctx.touchStartX) > 10) {
        e.preventDefault();
      }
    };
    window.addEventListener('touchmove', preventScroll, { passive: false, capture: true });
    
    const handleClick = (e) => {
      if (e.target.closest('#settings-panel') || e.target.closest('#settings-btn') || e.target.closest('#mute-btn') || e.target.closest('#tilt-btn') || e.target.closest('#mic-btn') || e.target.closest('#name-entry')) {
        return;
      }
      const ctx = getCtx();
      if (ctx.isPaused) {
        resumeGame();
        return;
      }
      if (ctx.gameOver && !ctx.showNameEntry && Date.now() - ctx.gameOverTime >= 1000) {
        startCountdown();
      }
      initAudio();
      tryStartBGMFromGesture();
    };
    window.addEventListener('click', handleClick);
    
    const handleKeydownBgm = (e) => {
      const ctx = getCtx();
      if (ctx.isPaused && e.key !== 'Escape') {
        resumeGame();
      }
      initAudio();
      tryStartBGMFromGesture();
    };
    window.addEventListener('keydown', handleKeydownBgm);
    
    const handleTouchstartBgm = () => {
      const ctx = getCtx();
      if (ctx.isPaused) {
        resumeGame();
      }
      initAudio();
      tryStartBGMFromGesture();
    };
    window.addEventListener('touchstart', handleTouchstartBgm, { passive: true });
    
    const handleResize = () => {
      const ctx = getCtx();
      if (ctx.camera && ctx.renderer) {
        ctx.camera.aspect = window.innerWidth / window.innerHeight;
        ctx.camera.updateProjectionMatrix();
        ctx.renderer.setSize(window.innerWidth, window.innerHeight);
      }
    };
    window.addEventListener('resize', handleResize);
    
    const handleKeydownSettings = (e) => {
      if (e.key === 'p' || e.key === 'P') {
        toggleSettings();
      }
    };
    window.addEventListener('keydown', handleKeydownSettings);
    
    let handleIOSOrientationTrigger = null;
    if (window.DeviceOrientationEvent) {
      if (typeof DeviceOrientationEvent.requestPermission === 'function') {
        handleIOSOrientationTrigger = () => {
          requestTiltPermission();
        };
        window.addEventListener('touchstart', handleIOSOrientationTrigger, { passive: true });
      } else {
        window.addEventListener('deviceorientation', handleDeviceOrientation);
      }
    } else {
      const ctx = getCtx();
      ctx.tiltEnabled = false;
      ctx.tiltEnabledRef = false;
    }
    
    window._gameControlsCleanup = () => {
      window.removeEventListener('keydown', handleKeyDown);
      window.removeEventListener('touchstart', handleTouchStart);
      window.removeEventListener('touchend', handleTouchEnd);
      window.removeEventListener('touchmove', preventScroll);
      window.removeEventListener('click', handleClick);
      window.removeEventListener('keydown', handleKeydownBgm);
      window.removeEventListener('touchstart', handleTouchstartBgm);
      window.removeEventListener('resize', handleResize);
      window.removeEventListener('keydown', handleKeydownSettings);
      if (handleIOSOrientationTrigger) {
        window.removeEventListener('touchstart', handleIOSOrientationTrigger);
      }
      window.removeEventListener('deviceorientation', handleDeviceOrientation);
    };
  });
  
  onUnmounted(() => {
    if (window._gameControlsCleanup) {
      window._gameControlsCleanup();
      window._gameControlsCleanup = null;
    }
  });

  const startTiltCalibration = () => {
    const ctx = getCtx();
    ctx.tiltCalibrationSamples = [];
    ctx.isCalibrating = true;
    ctx.tiltInitialBeta = null;
    ctx.tiltInitialGamma = null;
  };

  const finishTiltCalibration = () => {
    const ctx = getCtx();
    if (ctx.tiltCalibrationSamples.length === 0) {
      ctx.tiltInitialBeta = ctx.tiltInitialBeta ?? 45;
      ctx.tiltInitialGamma = ctx.tiltInitialGamma ?? 0;
    } else {
      const avgBeta = ctx.tiltCalibrationSamples.reduce((s, v) => s + v.beta, 0) / ctx.tiltCalibrationSamples.length;
      const avgGamma = ctx.tiltCalibrationSamples.reduce((s, v) => s + v.gamma, 0) / ctx.tiltCalibrationSamples.length;
      ctx.tiltInitialBeta = avgBeta;
      ctx.tiltInitialGamma = avgGamma;
    }
    ctx.isCalibrating = false;
    ctx.tiltCalibrationSamples = [];
  };

  return {
    handleJump,
    handleSlide,
    handleSwipe,
    handleTouchStart,
    handleTouchEnd,
    handleDeviceOrientation,
    handleKeyDown,
    updatePhysics,
    toggleTilt,
    startTiltCalibration,
    finishTiltCalibration
  };
}
