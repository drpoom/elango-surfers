<template>
  <div id="game-container">
    ...
    <div id="game-canvas" ref="container" tabindex="0"></div>
...
import { useNPCs } from './composables/useNPCs.js'
...
const initGame = () => {
  ...
  const gameSpeedRef = ref(0.25)
  const { spawnNPC: npcSpawn, updateNPCs, checkNPCCollision, clearNPCs, npcs } = useNPCs({ scene, getSurfaceY, gameSpeed: gameSpeedRef })
...
const triggerGameOver = (shakeIntensity = 0.5) => {
  ...
  obstacles.forEach(obs => { obs.mesh.traverse(c => { if (c.isMesh) { if (c.geometry && c.geometry !== sharedCoinGeo) c.geometry.dispose(); if (Array.isArray(c.material)) { c.material.forEach(m => m.dispose()); } else { c.material.dispose(); } } }); scene.remove(obs.mesh); });
  obstacles.length = 0;
...
const animate = () => {
  ...
  for (let index = obstacles.length - 1; index >= 0; index--) {
    ...
    if (Date.now() - gameStartTime < 2000) continue;
    ...
    if (horizDist < collisionDist && (hitGroundObs || hitFloatingObs)) {
      ...
      if (obs.obstacleType === 'slippery' && !isSlippery) {
        ...
        obs.mesh.traverse(c => { if (c.isMesh) { if (c.geometry && c.geometry !== sharedCoinGeo) c.geometry.dispose(); if (Array.isArray(c.material)) { c.material.forEach(m => m.dispose()); } else { c.material.dispose(); } } });
  ...
  for (let index = bonusCoins.length - 1; index >= 0; index--) {
    const bc = bonusCoins[index];
...
// Named event listeners for proper cleanup in onUnmounted
const handleGlobalError = (e) => { console.log('GLOBAL ERROR:', e.message, 'at', e.filename + ':' + e.lineno + ':' + e.colno); };
...
const handleKeydownP = (e) => {
  if (e.key === 'p' || e.key === 'P') {
    toggleSettings();
  }
};
const handleIosTiltTouchstart = () => {
...
onMounted(() => {
  ...
  window.addEventListener('keydown', handleKeydownResume);
  
  // Resume on touch if paused
  window.addEventListener('touchstart', handleTouchstartResume, { passive: true });
  window.addEventListener('resize', handleResize);
  
  // Pause on P key
  window.addEventListener('keydown', handleKeydownP);
  ...
  if (window.DeviceOrientationEvent) {
    if (typeof DeviceOrientationEvent.requestPermission === 'function') {
      ...
      window.addEventListener('touchstart', handleIosTiltTouchstart, { passive: true });
...
onUnmounted(() => {
  ...
  window.removeEventListener('keydown', handleKeydownResume);
  window.removeEventListener('touchstart', handleTouchstartResume);
  window.removeEventListener('resize', handleResize);
  window.removeEventListener('keydown', handleKeydownP);
  window.removeEventListener('touchstart', handleIosTiltTouchstart);
